...
//        if user is not logged in then redirect ti login page
        if (Auth::check() == false) {
...
//    echo $now->format('Y-m-d H:i:s');
    ...
    if ($code->starts_at != ""){
        $startData = Carbon::createFromFormat('Y-m-d H:i:s' ,$code->starts_at );
    if ($now->lt($startData)){
        return response()->json([
            'status' => false,
            'message' => 'Invalid discount coupon'
        ...
        if ($code->expires_at != ""){
            $endData = Carbon::createFromFormat('Y-m-d H:i:s' ,$code->expires_at );
            if ($now->gt($endData)){
                return response()->json([
                    'status' => false,
                    'message' => 'Invalid discount coupon'
